...
from globalgenie.docker.context import DockerBuildContext
...
if TYPE_CHECKING:
    from globalgenie.docker.resource.base import DockerResource
...
class DockerApp(InfraApp):
    ...
    # -*- GlobalGenie Volume
    # Mount ~/.config/gg directory from host machine to the container
    mount_globalgenie_config: bool = True
        ...
        container_env.update(
            {
                ...
                GLOBALGENIE_RUNTIME_ENV_VAR: "docker",
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        ...
        # Add ~/.config/gg as a volume
        if self.mount_globalgenie_config:
            globalgenie_config_host_path = str(Path.home().joinpath(".config/gg"))
            globalgenie_config_container_path = f"{self.workspace_dir_container_path}/.config/gg"
            logger.debug(f"Mounting: {globalgenie_config_host_path}")
            logger.debug(f"      to: {globalgenie_config_container_path}")
            container_volumes[globalgenie_config_host_path] = {
                "bind": globalgenie_config_container_path,
                "mode": "ro",
            }
    ...
    def build_resources(self, build_context: DockerBuildContext) -> List["DockerResource"]:
        from globalgenie.docker.resource.base import DockerResource
        from globalgenie.docker.resource.container import DockerContainer
        from globalgenie.docker.resource.network import DockerNetwork
    ...
    def get_infra_resources(self) -> Optional[Any]:
        from globalgenie.docker.resources import DockerResources
