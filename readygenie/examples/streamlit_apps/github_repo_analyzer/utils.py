...
def sidebar_widget() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("📋 Summarize 'globalgenie-agi/globalgenie'"):
            # Run this query in the current session
            add_message("user", "Summarize 'globalgenie-agi/globalgenie' repo")
        if st.button("🥇 List my recent repositories"):
            add_message("user", "List my recent repositories")
        if st.button("🏆 List latest issues in 'globalgenie-agi/globalgenie' "):
            add_message("user", "List latest issues in 'globalgenie-agi/globalgenie'")
        if st.button("🥇 List recent PRs in 'globalgenie-agi/globalgenie'"):
            add_message("user", "List recent PRs in 'globalgenie-agi/globalgenie' repo")
        # Chat controls
        st.header("Chat")
...
def about_widget() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown("""
        GlobalGenie is a comprehensive framework for building intelligent AI agents with advanced reasoning.

        [GitHub](https://github.com/globalgenie-agi/globalgenie) | [Docs](https://docs.globalgenie.com)
        """)

        st.markdown("### Need Help?")
        st.markdown(
            "If you have any questions, catch us on [discord](https://globalgenie.link/discord) or post in the community [forum](https://globalgenie.link/community)."
        )
