"""🤖 GlobalGenie Support Agent - Your AI Assistant for GlobalGenie Framework!

This example shows how to create an AI support assistant that combines iterative knowledge searching
with GlobalGenie's documentation to provide comprehensive, well-researched answers about the GlobalGenie framework.
...
- "How do I implement RAG with GlobalGenie?"

Run `pip install openai lancedb tantivy pypdf duckduckgo-search inquirer globalgenie` to install dependencies.
"""
...
def initialize_knowledge_base(load_knowledge: bool = False):
    """Initialize the knowledge base with GlobalGenie documentation
    ...
    if load_knowledge:
        ...
        print("   • Loading GlobalGenie documentation")
...
def get_agent_storage():
    ...
    return SqliteAgentStorage(
        table_name="globalgenie_assist_sessions", db_file="tmp/agents.db"
    )
...
def create_agent(
    ...
    return Agent(
        ...
        code to demonstrate GlobalGenie's capabilities in real-time.

        Your strengths include:
        - Deep understanding of GlobalGenie's architecture and capabilities
        - Access to GlobalGenie documentation and API reference, search it for relevant information
        - Creating and testing working GlobalGenie Agents
        ...
        """),
        instructions=dedent("""\
        Your mission is to provide comprehensive, hands-on support for GlobalGenie developers
        ...
        Key GlobalGenie Concepts to Emphasize:
        - Agent levels (0-3) and capabilities
        - Multimodal and streaming support
        - Model globalgeniesticism and provider flexibility
        ...
        - Focus on GlobalGenie's core principles: Simplicity, Performance, and GlobalGeniesticism
        - Save code examples to files when they would be useful to run"""),
        knowledge=agent_knowledge,
        tools=[PythonTools(base_dir=tmp_dir.joinpath("globalgenie_assist"), read_files=True)],
...
def globalgenie_support_agent(
    ...
    table.add_row("🤖 Welcome to [bold green]GlobalGenieAssist[/bold green]")
    table.add_row("Your Personal GlobalGenie Expert")
    console.print(table)
...
if __name__ == "__main__":
    typer.run(globalgenie_support_agent)
