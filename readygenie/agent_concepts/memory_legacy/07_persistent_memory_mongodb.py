...
Steps:
1. Run: `pip install openai pymongo globalgenie` to install dependencies
2. Make sure you are running a local instance of mongodb
3. Run: `python readygenie/memory/07_persistent_memory_mongodb.py` to run the agent
"""
...
from globalgenie.memory.db.mongodb import MongoMemoryDb
from globalgenie.models.openai import OpenAIChat
from globalgenie.storage.agent.mongodb import MongoDbAgentStorage
...
agent = Agent(
    ...
    storage=MongoDbAgentStorage(
        collection_name="agent_sessions", db_url=db_url, db_name="globalgenie"
    ),
