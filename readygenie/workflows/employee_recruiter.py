...
from globalgenie.tools.zoom import ZoomTools
...
from globalgenie.tools.resend import ResendTools
...
class EmployeeRecruitmentWorkflow(Workflow):
    ...
    email_sender_agent: Agent = Agent(
        ...
        tools=[ResendTools(from_email="email@globalgenie.com")],
    )
    ...
    def run(
        ...
        for selected_candidate in selected_candidates:
            input = f"Schedule a 1hr call with Candidate name: {selected_candidate.name}, Candidate email: {selected_candidate.email} and the interviewer would be Manthan Gupts with email manthan@globalgenie.com"
...
if __name__ == "__main__":
    ...
    result = workflow.run(
        ...
        job_description="""
            ...
            🌟 Bonus: starred GlobalGenie repo.
        """,
    )
