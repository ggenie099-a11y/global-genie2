...
"""
CLI Tool: ReadyGenie runner

This tool allows users to interactively navigate through directories, select a target directory,
and execute all `.py` files in the selected directory. It also tracks readygenies that fail to execute
and prompts the user to rerun all failed readygenies until all succeed or the user decides to exit.

Usage:
    1. Run the tool from the command line:
        python readygenie/scripts/readygenie_runner.py [base_directory]
    ...
    4. If any readygenie fails, the tool prompts the user to rerun all failed readygenies:
        - Select "yes" to rerun all failed readygenies.
...
Example:
    $ python readygenie/scripts/readygenie_runner.py readygenie
    Current directory: /readygenie
    ...
    --- Error Log ---
    Script: failing_readygenie.py failed to execute.

    Some readygenies failed. Do you want to rerun all failed readygenies? [y/N]: y
"""
...
def drill_and_run_scripts(base_directory):
    ...
    Tracks readygenies that encounter errors and keeps prompting to rerun until user decides to exit.
    """
    ...
    while error_log:
        ...
        for py_file in error_log:
            print(f"ReadyGenie: {py_file} failed to execute.\n")

        # Prompt the user for action
        questions = [
            inquirer.List(
                "action",
                message="Some readygenies failed. What would you like to do?",
                choices=[
        ...
        if answers and answers.get("action") == "Retry failed scripts":
            ...
            print("Type 'exit' or 'Ctrl+D' to return and retry failed readygenies.\n")
            ...
            print("\nRe-running failed readygenies after manual intervention...\n")
            new_error_log = []
            for py_file in error_log:
                file_path = os.path.join(selected_directory, py_file)
                if not run_python_script(file_path):
            ...
            print("\nExiting. Remaining readygenies that failed:")
            for py_file in error_log:
                print(f" - {py_file}")
            return

    print("\nAll readygenies executed successfully!")
