...
from globalgenie.tools.e2b import E2BTools
...
from globalgenie.vectordb.lancedb.lance_db import LanceDb
...
globalgenie_assist_knowledge = UrlKnowledge(
...
globalgenie_assist = Agent(
    ...
    role="You help answer questions about the GlobalGenie framework.",
    model=OpenAIChat(id="gpt-4o"),
    instructions="Search your knowledge before answering the question. Help me to write working code for GlobalGenie Agents.",
    tools=[
        KnowledgeTools(
            knowledge=globalgenie_assist_knowledge, add_instructions=True, add_few_shot=True
        ),
    ],
...
if __name__ == "__main__":
    ...
    # asyncio.run(globalgenie_assist_knowledge.aload())

    # asyncio.run(agent_team.aprint_response("Hi! What are you capable of doing?"))

    # Python code execution
    # asyncio.run(agent_team.aprint_response(dedent("""What is the right way to implement an GlobalGenie Agent that searches Hacker News for good articles?
    ...
    #                                        Don't mock anything. Use the real information from the GlobalGenie documentation."""), stream=True))
    ...
    # Github analysis
    # asyncio.run(agent_team.aprint_response(dedent("""List open pull requests in the globalgenie-agi/globalgenie repository.
    ...
    asyncio.run(
        agent_team.aprint_response(
            dedent(f"""I have a patient with the following medical information:\n {loaded_txt}
                                                    What is the most likely diglobalgeniesis?
                                                """),
            stream=True,
        )
    )
